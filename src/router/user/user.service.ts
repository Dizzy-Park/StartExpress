...
import { getToken } from '/module/auth';
import { decrypt, encript, getIp, getRandomToken, isPassword } from '/module/security';
import { IPayload, createError, createPayload, reqPost } from '/entity/common';

/**
 * 로그인 검증
 */
export const login = async (req: Request, res: Response, next: NextFunction) => {
  const payload: IPayload<{ token?: string; key: string }> = createPayload<{
    token: string;
    key: string;
  }>();
  try {
    ...
    if (info === undefined) {
      ...
      if (info.u_status !== '0') {
        ...
        const ps: boolean = await isPassword(pwd as string, info.salt, info.pass);
 ...
 */
export const securityKey = async (req: Request, res: Response, next: NextFunction) => {
